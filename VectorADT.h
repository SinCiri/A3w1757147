...
// overloaded binary search that has additional parameters,this is recursive binary search
  int  binarySearchByTitle(int& steps, T * v, int first, int last);
  // binary search that calls the overloaded function with first =0 and last = count-1; The data v contains the title being searched
...
//radix sort - see Zybooks for algorithm, also watch the given video resource
 ...
 //recursive merge sort - see Zybooks for algorithm, also watch the given video resource 
